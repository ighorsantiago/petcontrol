...
// ─── Google (via expo-auth-session — sem módulo nativo) ───────────────────────

// Recebe o accessToken retornado pelo hook Google.useAuthRequest nas telas
export async function signInWithGoogle(accessToken: string): Promise<User> {
    try {
